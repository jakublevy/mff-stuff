...
namespace _087fireworks
{
  ...
  public class Particle : DefaultRenderObject
  {
    ...
    public Particle ( Vector3d pos, Vector3d vel, Vector3d _up, Vector3 col, double siz, double time, double age, bool explosive = true)
    {
      position = pos;
      velocity = vel;
      up = _up;
    ...
    public Vector3d Norm (Vector3d v)
    {
      double size = VecSize(v);
      return new Vector3d(v.X / size, v.Y / size, v.Z / size);
    }

    public double VecSize (Vector3d v) => Math.Sqrt(v.X * v.X + v.Y * v.Y + v.Z * v.Z);
    ...
    public bool Simulate ( double time, Fireworks fw )
    {
      ...
      {
        if(explosive)
          ExplosionAdd(time, fw);
        return false;
      }

      if (VecSize(velocity) < 0.4 && explosive)
      {
          ExplosionAdd(time, fw);
        return false;
      }

      if (position.Y > 5 && explosive)
      {
        ExplosionAdd(time, fw);
        return false;
      }
      ...
      Vector3d g = new Vector3d(0, -0.0981f, 0);
      ...
      if ( fw.particleDynamic )
      {
          ...
          //odporova sila
          Vector3d opp = -velocity;
          opp = Norm(opp);
          double speed = VecSize(velocity);
          opp = dt * opp * (0.01 * speed + 0.01 * speed * speed);
          velocity += opp;
    ...
    private Vector3d RandomDirVector ()
    {
      double r = NextDouble();
      double x = -1 + r * 2;
      r = NextDouble();
      ...
      double z = 1 - Math.Sqrt(x * x + y * y);
      return new Vector3d(x,y,z);
    }

    private void ExplosionAdd (double time, Fireworks fw)
    {
      RandomGenerator rg = new RandomGenerator();
      int particlesC = rg.Next(fw.explosionParticlesCountMin, fw.explosionParticlesCountMax+1);
      for (int i = 0; i < particlesC; ++i)
      {
        Vector3d dir = RandomDirVector();
        Particle p = new Particle(position, dir * NextDouble(0.2,0.5), up,
          new Vector3((float) NextDouble(0.1f, 1.0f), (float)NextDouble(0.1f, 1.0f),(float)NextDouble(0.1f, 1.0f)),
          NextDouble(1,3), time,  NextDouble(1,4),false
          );
        fw.AddParticle(p);
      }
  ...
  public class Fireworks
  {
    ...
    /// </summary>
    public List<int> expiredParticles;
    ...
    public int explosionParticlesCountMin = 20;
    public int explosionParticlesCountMax = 50;
    ...
    public void Update ( string param )
    {
      ...
      if (launchersCount != 1)
      {
        double step = 12.0 / (launchersCount - 1);
        for (int i = 0; i < launchersCount; ++i)
        {
          k = new Launcher(freq, new Vector3d(-6 + step * i, -2.0, 0.0), null, new Vector3d(0.5, 0.0, 0));
          AddLauncher(k);
        }
      }
      else
      {
        k = new Launcher(freq, new Vector3d(0, -2.0, 0.0), null, new Vector3d(0.5, 0.0, 0));
        AddLauncher(k);
      }


      // launchers: frequency
      if ( Util.TryParse( p, "secondsPerHit", ref freq ) )
      {
        if ( freq < 0 )
          freq = 3;
      ...
      if (Util.TryParse(p, "explosionParticlesCountMin", ref explosionParticlesCountMin))
      {
        if (explosionParticlesCountMin < 0)
        {
          explosionParticlesCountMin = 20;
        }
      }

      if (Util.TryParse(p, "explosionParticlesCountMax", ref explosionParticlesCountMax))
      {
        if (explosionParticlesCountMax < explosionParticlesCountMin)
        {
          var tmp = explosionParticlesCountMin;
          explosionParticlesCountMin = explosionParticlesCountMax;
          explosionParticlesCountMax = tmp;
  ...
  public partial class Form1
  {
    ...
    static void InitParams ( out string param, out string tooltip, out string name, out MouseButtons trackballButton, out Vector3 center, out float diameter,
                             out bool useTexture, out bool globalColor, out bool useNormals, out bool usePtSize, out int launchersCount)
    {
      //secondsPerHit=2,launchers=3,explosionParticlesCountMin=20,explosionParticlesCountMax=50,variance=0.1,max=60000,slow=0.25,dynamic=1,ticks=0

      param = "secondsPerHit=10,launchers=13,explosionParticlesCountMin=20,explosionParticlesCountMax=50,variance=0.1,max=60000,slow=0.25,dynamic=1,ticks=0";
      tooltip = "secondsPerHit,launchers,explosionParticlesCountMin,explosionParticlesCountMax,max,slow,dynamic,variance,ticks,screencast";
      ...
      //secondsPerHit
      //  number of seconds to wait between launching next rocket

      //launchers
      //  number of launchers that will launch rockets

      //explosionParticlesCountMin
      //  minimium number of particles that will be created by an explosion

      //explosionParticlesCountMax
      //  maximum number of particles that will be created by an explosion
